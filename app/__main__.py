...
def get_all_patient_data() -> list[Patient]:
    patient_client = ClinicsCardPatient(
        http_client=Client(base_url="https://cliniccards.com/api"), api_key=settings.CLINICS_CARD_API_KEY
    )
...
def inser_not_exist_patients_excel(patients: list[Patient]):
    ...
    for patient in patients:
        ...
        def get_inisert_patient_values(patient: Patient):
            ...
            return [
                "", full_name, patient.code, patient.curator, first_doctor, visits_count, "", treatment_plan
            ]
            
        if not is_patient_exist:
            inser_patint_values = get_inisert_patient_values(patient=patient)
            google_sheet_client.insert_row_at(row_index=patient_row_position, values=inser_patint_values)
            logger.info("Insert new patient %s position %s", patient.code, patient_row_position)
            logger.info("Insert new patient %s values %s", patient.code,inser_patint_values)
            
        else:
            treatment_plan = patient.main_plans.plan_total_with_discount if patient.main_plans else ""
            treatment_plan = int(float(treatment_plan)) if treatment_plan else ""
            google_sheet_client.update_element_at(ColumnElementId.TREATMENT_PLAN.value, patient_row_position, treatment_plan)
        ...
        for patient_payment, payment_sum in patient_payment_sums.items():
            payment_date_position = get_payment_date_position(patient_payment.date_created, google_sheet_client=google_sheet_client)
            patient_payment_date_position = (payment_date_position[0], patient_row_position)
        
            google_sheet_client.update_element_at(*patient_payment_date_position, payment_sum)
            logger.info("Insert patient %s payment %s by the date: %s", patient.code, payment_sum, patient_payment_date_position)
            time.sleep(1)
